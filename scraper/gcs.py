"""Pure HTTP client for Google Cloud Storage via the XML API."""
import logging
...
class GCSClient:
    """Pure HTTP client for GCS. No caching, no filesystem awareness."""
    ...
    def fetch_text(self, path: str) -> Optional[str]:
        """Fetch a GCS object as text. Returns None on 404."""
    ...
    def fetch_bytes(self, path: str) -> Optional[bytes]:
        """Fetch a GCS object as bytes. Returns None on 404."""
        url = f"{GCS_BASE}/{self.bucket}/{path}"
    ...
    def stream(self, path: str) -> Optional[Iterator[bytes]]:
        """Stream a GCS object in chunks. Returns None on 404.

        The caller must consume the iterator to release the connection.
        """
        url = f"{GCS_BASE}/{self.bucket}/{path}"
        log.debug("GET %s (stream)", url)
        resp = self.session.get(url, timeout=300, stream=True)
        if resp.status_code == 404:
            resp.close()
            return None
        resp.raise_for_status()
        return resp.iter_content(chunk_size=256 * 1024)
