...
def _mock_known_build_ids(build_ids):
    """Mock the VM label values API to return known build_ids."""
    responses.add(responses.GET, f"{VM_URL}/api/v1/label/build_id/values",
...
def test_scrape_discovers_and_ingests(metrics_json):
    ...
    scraper = Scraper(gcs, session, VM_URL, pipelines, workers=1)

    scraper.scrape(base_path, since=now - 3600, until=now + 3600, dry_run=False)

    post_calls = [c for c in responses.calls if c.request.method == "POST"]
    assert len(post_calls) > 0
    # Verify sentinel was pushed (last POST should contain ci_build_scraped)
    sentinel_calls = [c for c in post_calls if "ci_build_scraped" in (c.request.body or "")]
